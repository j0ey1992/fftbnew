...
import { getContractTemplate, updateContractTemplate } from '@/lib/firebase/contract-templates'

/**
 * Admin page for editing a contract template
 */
export default function EditContractTemplatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        ...
        const template = await getContractTemplate(resolvedParams.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update template
      const resolvedParams = await params
      await updateContractTemplate(resolvedParams.id, {
        name,
        description,
      ...
      setError('Failed to update contract template')
    } finally {
      setLoading(false)
    }
  }
            ...
            <div className="p-6">
              ...
              {success && (
                <div className="mb-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg text-green-400">
                  Contract template updated successfully! Redirecting...
                </div>
              )}
