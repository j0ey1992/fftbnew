...
interface LpStakingProductCardProps {
  id: string
  name: string
  image: string
  apr: string
...
export function LpStakingProductCard({
  id,
  name,
  image,
  apr,
        ...
        <div className="relative flex flex-col justify-between h-full p-4 z-10">
          <div>
            ...
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                {pairInfo && (
                  <div className="text-xs text-white/70 mt-0.5">{pairInfo}</div>
                )}
              </div>
          ...
          <div className="flex justify-between items-center mt-2">
            <div className="flex items-center">
              <div className="w-8 h-8 rounded-full bg-[#051326] p-1 flex items-center justify-center">
                <Image
                  src={image}
                  alt={name}
                  width={32}
              ...
              {isNew && (
                <span className="ml-2 bg-blue-600/30 text-blue-400 text-xs px-2 py-0.5 rounded">New</span>
              )}
            </div>
