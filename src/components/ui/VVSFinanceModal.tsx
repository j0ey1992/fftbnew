...
interface VVSFinanceModalProps {
  isOpen: boolean
  onClose: () => void
}

export function VVSFinanceModal({ isOpen, onClose }: VVSFinanceModalProps) {
  ...
  useEffect(() => {
    const fetchTrade = async () => {
      ...
      try {
        ...
        // For buying, we're using CRO as the input token when buying other tokens
        const inputToken = selectedCrypto.isNative ? cryptocurrencies[1].address : 'NATIVE' // Use WCRO if buying CRO, otherwise use native CRO
        const outputToken = selectedCrypto.address
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      /*
      const provider = new ethers.BrowserProvider(window.ethereum, BuiltInChainId.CRONOS_MAINNET)
      const signer = await provider.getSigner()
      
      const approvalTx = await approveIfNeeded(BuiltInChainId.CRONOS_MAINNET, tradeDetails, signer)
      if (approvalTx) {
        await approvalTx.wait()
      }
      
      const tx = await executeTrade(BuiltInChainId.CRONOS_MAINNET, tradeDetails, signer)
      await tx.wait()
      */

      // For now, just show a message since we don't have wallet connection
      ...
      const outAmount = typeof tradeDetails.amountOut === 'object' ? 
        (tradeDetails.amountOut.amount || 
         (tradeDetails.amountOut.numerator && tradeDetails.amountOut.denominator ? 
          ...
          '0')) : 
        String(tradeDetails.amountOut);
        
      alert(`This would execute a real trade using VVS Finance. Input: ${inAmount} ${selectedCrypto.isNative ? 'WCRO' : 'CRO'}, Output: ${outAmount} ${selectedCrypto.symbol}`)

      // Reset the interface after successful swap
      setShowSwapInterface(false)
      setSelectedCrypto(null)
      setSwapAmount('')
      setTradeDetails(null)
            ...
            <GlassCard elevation="raised" className="overflow-hidden">
              ...
              <div className="p-5 bg-[#0a0f1f] text-white flex justify-between items-center border-b border-white/5">
                <h3 className="text-xl font-bold">
                  {showSwapInterface ? `Buy ${selectedCrypto?.symbol}` : 'Buy Cryptocurrency'}
                </h3>
                <button
                  onClick={onClose}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400">
                        {selectedCrypto?.isNative ? 'CRO' : 'CRO'}
                      </div>
                    </div>
                    <p className="text-gray-500 text-xs mt-1">
                      Enter the amount of {selectedCrypto?.isNative ? 'WCRO' : 'CRO'} to swap for {selectedCrypto?.symbol}
                    </p>
                  </div>
                            ...
                            {typeof tradeDetails.amountIn === 'object' ? 
                              (tradeDetails.amountIn.amount || 
                               (tradeDetails.amountIn.numerator && tradeDetails.amountIn.denominator ? 
                                String(Number(tradeDetails.amountIn.numerator) / Number(tradeDetails.amountIn.denominator)) : 
                                '0')) : 
                              String(tradeDetails.amountIn)} {selectedCrypto?.isNative ? 'WCRO' : 'CRO'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                            ...
                            {typeof tradeDetails.amountOut === 'object' ? 
                              (tradeDetails.amountOut.amount || 
                               (tradeDetails.amountOut.numerator && tradeDetails.amountOut.denominator ? 
                                String(Number(tradeDetails.amountOut.numerator) / Number(tradeDetails.amountOut.denominator)) : 
                                '0')) : 
                              String(tradeDetails.amountOut)} {selectedCrypto?.symbol}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400 text-sm">Exchange rate</span>
                          <span className="text-white text-sm">
                            1 {selectedCrypto?.isNative ? 'WCRO' : 'CRO'} ≈ {
                              (() => {
                                const inAmount = typeof tradeDetails.amountIn === 'object' ? 
