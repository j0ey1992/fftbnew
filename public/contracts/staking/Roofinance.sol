...
/* ------------------------------- RooFinanceV1 ------------------------------ */
contract RooFinanceV1 is ReentrancyGuard, Pausable, Ownable, IERC721Receiver {
    ...
    function emergencyWithdraw(address collection) external nonReentrant {
        ...
        // Create arrays to track successful and failed transfers
        uint256[] memory successfulTransfers = new uint256[](tokenCount);
        uint256[] memory failedTransfers = new uint256[](tokenCount);
        uint256 successCount = 0;
        uint256 failCount = 0;
        
        // Try to transfer each NFT
        for (uint256 i = 0; i < tokenCount; i++) {
            uint256 tokenId = userTokens[i];
            
            try IERC721(collection).safeTransferFrom(address(this), msg.sender, tokenId) {
                successfulTransfers[successCount] = tokenId;
                successCount++;
            } catch {
                failedTransfers[failCount] = tokenId;
                failCount++;
                emit EmergencyWithdrawFailed(msg.sender, collection, tokenId);
            }
        }
        
        // Only update state for successfully transferred NFTs
        if (successCount > 0) {
            // Update balances
            totalStaked[collection] -= successCount;
            collectionToUserToBalances[collection][msg.sender] -= successCount;
            
            // Remove successful transfers from user's staked array
            for (uint256 i = 0; i < successCount; i++) {
                uint256 tokenId = successfulTransfers[i];
                
                // Find and remove the token from the staked array
                uint256 idx = stakedTokenIndex[collection][msg.sender][tokenId];
                uint256 lastIndex = usersNftStaked[collection][msg.sender].length - 1;
        ...
        // If some transfers failed, the user can try again later
        if (failCount > 0) {
            emit EmergencyWithdrawPartialFailure(msg.sender, collection, successCount, failCount);
        }
        
        emit EmergencyWithdraw(msg.sender, collection, successCount);
    }
     ...
     * This function ensures that reward tokens can only be recovered if they exceed
    ...
    event EmergencyWithdrawPartialFailure(address indexed user, address indexed collection, uint256 successCount, uint256 failCount);
}